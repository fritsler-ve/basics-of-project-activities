...
namespace ConsoleApp
{
    class Program
    {
        ...
        private static string SolveExpression(ref List<string> data)
        {
            ...
            char temp;
            for (var i = 0; i < data.Count; i++) // ZALOOPA
            {
                temp = data[i][0];
            ...
            for (var i = 0; i < data.Count; i++) // может показаться что вылезет за границы массива, 
...
/*
            ...
            string answer = SolveThree(newTask.Question) + "";
            ...
            updatedTask.TeamAnswer = SolveThree(updatedTask.Question) + "";
            Console.WriteLine($"  Ответ команды: {SolveThree(updatedTask.Question)}");
            Console.WriteLine();
